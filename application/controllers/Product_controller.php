...
class Product extends CI_controller{
	...
	function report_results()
	{
		...
		if($this -> access_control_model -> my_access($data['user_type'], 'Product', 'catagory_entry'))
		{
			$data['sale_status'] = '';
	...
	******************************/
	function transfer_product()
	{
		$timezone = "Asia/Dhaka";
			...
			foreach($query -> result() as $field):
				$product[ base_url().'index.php/Product/transfer_product/'.$field -> product_id ] = $field -> product_name;
			endforeach;
	...
	{
		$data['user_type'] = $this->tank_auth->get_usertype();
		if($this -> access_control_model -> my_access($data['user_type'], 'Product', 'purchase_entry'))
		{
			$timezone = "Asia/Dhaka";
			date_default_timezone_set($timezone);
			$bd_date = date('Y-m-d');
			...
			$product[''] = 'Select A Product';
			foreach($query -> result() as $field):
			 $product[base_url().'Product/purchase_entry/'.$field -> product_id] = $field -> product_name;
			endforeach;
	...
	function create_purchase()
	{
		...
		if($this -> access_control_model -> my_access($data['user_type'], 'Product', 'purchase_entry'))
		{
			...
			$config['base_url'] = base_url().'index.php/Product/create_purchase'. $pro_id ;
			$config['per_page'] = 15;
		    $config['num_links'] = 5;
	...
	{
		$data['user_type'] = $this->tank_auth->get_usertype();
		if($this -> access_control_model -> my_access($data['user_type'], 'Product', 'sale_price'))
		{
			//* for sale Running Status */
			$data['sale_status'] = '';
			/* end of Sale running Status*/
			...
			foreach($temp -> result() as $field):
				$product[ base_url().'index.php/Product/sale_price/'.$field -> product_id ] = $field -> product_name;
			endforeach;
			$data['product_info'] = $product; 
			if($this -> uri -> segment(3))
			{
	...
	{
		/*$data['user_type'] = $this->tank_auth->get_usertype();
		if($this -> access_control_model -> my_access($data['user_type'], 'Product', 'shelf_set_up'))
		{*/
			$data['sale_status'] = '';
			$data['user_type'] = $this->tank_auth->get_usertype();
	...
	{
		$data['user_type'] = $this->tank_auth->get_usertype();
		if($this -> access_control_model -> my_access($data['user_type'], 'Product', 'store_in_shelf_entry'))
		{
			$data['sale_status'] = '';
		...
		if($this -> access_control_model -> my_access($data['user_type'], 'Product', 'store_in_shelf_entry'))
		{
			...
			else
			{
				...
				if($this -> product_model -> create_store_in_shelf())
				{
					$data['status'] = 'successful';
					$this -> load -> view('include/template', $data);
					redirect('Product/store_in_shelf_entry/successful');
				}
				else
				{
					$data['status'] = 'failed';
	...
	{
		$data['user_type'] = $this->tank_auth->get_usertype();
		if($this -> access_control_model -> my_access($data['user_type'], 'Product', 'change_product_status'))
		{
			$data['sale_status'] = '';
			$data['status'] = '';
			$data['user_name'] = $this->tank_auth->get_username();
			...
			$product[''] = 'Select A Product';
			foreach($temp -> result() as $field):
				$product[ base_url().'index.php/Product/change_product_status_form/'.$field -> product_id ] = $field -> product_name;
			endforeach;
			$data['product_info'] = $product; 
			if($this -> uri -> segment(3))
			{
	...
	{
		$data['user_type'] = $this->tank_auth->get_usertype();
		if($this -> access_control_model -> my_access($data['user_type'], 'Product', 'alarming_level'))
		{
			$data['sale_status'] = '';
			$data['status'] = '';
			$data['user_name'] = $this->tank_auth->get_username();
			...
			$product[''] = 'Select A Product';
			foreach($temp -> result() as $field):
				$product[ base_url().'index.php/Product/alarming_level/'.$field -> product_id ] = $field -> product_name;
			endforeach;
			$data['product_info'] = $product; 
			if($this -> uri -> segment(3))
			{
	...
	{
		$data['user_type'] = $this->tank_auth->get_usertype();
		if($this -> access_control_model -> my_access($data['user_type'], 'Product', 'warranty_period'))
		{
			//* for sale Running Status */
			$data['sale_status'] = '';
			/* end of Sale running Status*/
			...
			$product[''] = 'Select A Product';
			foreach($temp -> result() as $field):
				$product[ base_url().'index.php/Product/warranty_period/'.$field -> product_id ] = $field -> product_name;
			endforeach;
			$data['product_info'] = $product; 
			if($this -> uri -> segment(3))
			{
	...
	function salePriceEntry(){
		...
		$query = $this -> joy_model -> productInfoGeneral(FALSE, 0);
		$product[base_url().'Product/salePriceEntry/'.$this -> uri -> segment(3)] = 'Select A Product';
		foreach($query -> result() as $field):
			 $product[base_url().'Product/salePriceEntry/'.$field -> product_id] = $field -> product_name;
		endforeach;
		...
		function my_sale_add()
		{
			...
			else redirect('Product/purchase_entry/'.$inputValue);
		}
		function sale_price_entry_barcode()
		{
			...
			else redirect('Product/salePriceEntry/'.$inputValue);
		}
		...
		function create_damage()
		{
			...
			{
				$data['status'] = 'success';
				redirect('Product/all_stock_for_damage_entry/success');
			}
			else
			{
				$data['status'] = 'failed';
				redirect('Product/all_stock_for_damage_entry/failed');
			}	
		}
		function all_damage_stock()
		{
			...
			$config['base_url'] = base_url().'index.php/Product/all_damage_stock/';
			...
			foreach($query -> result() as $field):
				 $temp[base_url().'Product/all_damage_stock/'.$field->product_id] = $field -> product_name;
			endforeach;
